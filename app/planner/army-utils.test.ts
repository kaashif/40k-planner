...
test('full supplied roster has correctly sized bases, unique IDs and the requested reserves',()=>{
 ...
 assert.equal(all.length,49);assert.equal(staged.markers.length,37);assert.equal(staged.deepStrikeMarkers.length,12);assert.equal(new Set(all.map(m=>m.id)).size,49);
