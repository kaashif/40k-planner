...
export default function ThreatPercentile({value,onChange,ruleTags=[]}:{value:ThreatSettings;onChange:(value:ThreatSettings)=>void;ruleTags?:string[]}){
 ...
 const advance=selectedProbabilityRange(effective,'advance'),charge=selectedProbabilityRange(effective,'charge');
 return <section className="threat-percentile" aria-label="Threat percentile">
  <label htmlFor="threat-percentile">Percentile <strong>{percent}%{percent===50?' · median':''}</strong></label>
  <input id="threat-percentile" aria-label="Threat percentile" title="Chance of reaching the displayed total; higher percentages give shorter, safer reach" type="range" min="1" max="100" step="1" value={percent} onChange={e=>onChange({...value,percentile:Number(e.target.value)})}/>
  <div className="percentile-distances"><span>Advance <b>{advance}″</b></span><span>Charge <b>{charge>0?`${charge}″`:'—'}</b></span></div>
  ...
  <small>Scout {effective.useScout?effective.scout:0}″ fixed · Move {effective.move}″ · totals above</small>
