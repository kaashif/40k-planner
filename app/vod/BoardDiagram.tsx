...
export default function BoardDiagram({second,board,previousBoard,gameId}: {second:number;board:BoardState;previousBoard?:{second:number;board:BoardState};gameId:string}) {
  ...
  return <section className="vod-board" aria-label="Simplified board-state diagram">
    ...
    <svg viewBox="0 0 640 480" role="img" aria-labelledby={`${id}-title ${id}-description`}>
      ...
      <desc id={`${id}-description`}>{`Teal: Thousand Sons; pink: opponent. Terrain and positions approximate. ${board.groups.map(g=>g.label).join(', ')}. Arrows connect observed positions, not verified continuous routes.`}</desc>
      ...
      <g transform="translate(20 20)">
        {board.terrain.map((r,i)=><g key={i} transform={`translate(${r.x*6} ${r.y*4.4}) rotate(${r.r} ${r.w*3} ${r.h*2.2})`}><rect width={r.w*6} height={r.h*4.4} fill="#26363d" stroke="#4a5b60"/><path d={`M4 ${r.h*4.4-4} V4 H${r.w*6-4}`} stroke="#718084" strokeWidth="4" fill="none"/></g>)}
